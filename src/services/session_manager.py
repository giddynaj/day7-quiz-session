...
class SessionManager:
    ...
    async def create_session(self, user_id: str, quiz_id: str) -> QuizAttempt:
        ...
        # Store in PostgresSQL for persistence
        async with db.pg_pool.acquire() as conn:
            await conn.execute('''
                INSERT INTO quiz_attempts
                (id, user_id, quiz_id, started_at, current_question,
                answers, status, time_remaining, version)
        ...
        async with db.pg_pool.acquire() as conn:
            async with conn.transaction():
                ...
                result = await conn.execute('''
                    ...
                    SET answers = $1, current_question = $2, last_updated = NOW(),
                    version = $4 WHERE id = $3 AND version $5''', json.dumps(answers),
                    question_id, session_id, new_version, current['version'])
